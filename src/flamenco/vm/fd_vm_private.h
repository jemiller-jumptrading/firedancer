...
#define FD_MAX_ACCOUNT_DATA_LEN                    (10UL * 1024UL * 1024UL)
#define FD_MAX_ACCOUNT_DATA_GROWTH_PER_TRANSACTION (FD_MAX_ACCOUNT_DATA_LEN * 2UL)
...
ulong           sz,
  ...
  /* If the requested length is greater than the size of the region, resize the region
  ...
  if( FD_UNLIKELY( requested_len > region->region_sz ) ) {
    ...
    ulong new_region_sz = fd_ulong_min(
      fd_ulong_min( region->address_space_reserved, FD_MAX_ACCOUNT_DATA_LEN ),
    ...
    fd_memset( (void *)(region->haddr + region->region_sz), 0, new_region_sz - region->region_sz );
