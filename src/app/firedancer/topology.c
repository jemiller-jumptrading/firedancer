...
resolve_peer( char const *            peer,
  ...
  if( FD_LIKELY( FD_UNLIKELY( !colon && !https )  ) ) {
    ...
    host_end = host_port + strnlen( host_port, 256UL-(ulong)(host_port-peer) );
  }
...
#define MAX_RESOLVED_ADDRS 8UL

    for( ulong j=0UL; j<peers_cnt; j++ ) {
      if( FD_UNLIKELY( !config->firedancer.snapshots.sources.http.peers[ j ].enabled ) ) continue;

      fd_ip4_port_t resolved_addrs[ MAX_RESOLVED_ADDRS ];
      struct addrinfo hints = { .ai_family = AF_INET, .ai_socktype = SOCK_STREAM };
      int num_resolved = resolve_peer( config->firedancer.snapshots.sources.http.peers[ j ].url,
                                       &hints,
                                       "snapshots.sources.http.peers",
      ...
      if( FD_UNLIKELY( 0==num_resolved ) ) {
        ...
        if( FD_UNLIKELY( (ulong)num_resolved+resolved_peers_cnt>=FD_TOPO_SNAPRD_HTTP_PEERS_MAX ) ) continue;
        for( ulong i=0UL; i<(ulong)num_resolved; i++ ) tile->snaprd.http.peers[ resolved_peers_cnt+i ].addr = resolved_addrs[ i ];
