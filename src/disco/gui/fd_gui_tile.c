...
during_frag( fd_gui_ctx_t * ctx,
  ...
  if( FD_UNLIKELY( ctx->in_kind[ in_idx ]==IN_KIND_SHRED_OUT ) ) {
    ...
    return;
      ...
      fd_replay_votes_t * votes = (fd_replay_votes_t *)src;
      FD_TEST( votes->vote_cnt <= FD_RUNTIME_MAX_VOTE_ACCOUNTS );
      ulong actual_sz = sizeof(votes->vote_cnt)+(votes->vote_cnt*sizeof(votes->snapshot[0]));
      FD_TEST( sz==(ushort)(actual_sz) );
      sz = actual_sz;
  }
...
after_frag( fd_gui_ctx_t *      ctx,
  ...
  switch ( ctx->in_kind[ in_idx ] ) {
    ...
    case IN_KIND_REPLAY_VOTES: {
      FD_TEST( ctx->is_full_client );
      fd_replay_votes_t * votes = (fd_replay_votes_t *)ctx->buf;
      FD_TEST( votes->vote_cnt <= FD_RUNTIME_MAX_VOTE_ACCOUNTS );
      fd_gui_peers_handle_vote_update( ctx->peers, votes, fd_clock_now( ctx->clock ) );
      break;
    }
