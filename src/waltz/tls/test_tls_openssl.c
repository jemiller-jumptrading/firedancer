...
test_server( SSL_CTX * ctx ) {
  ...
  *server = (fd_tls_t) {
  ...
  ulong tp_sz = sizeof(tp_buf);
  FD_TEST( 1==SSL_set_quic_tls_transport_params( ssl, tp_buf, tp_sz ) );
  ...
  /* Process handshake messages back and forth until connected.
     This handles all cases: with/without HelloRetryRequest, etc. */
  while( hs->base.state != FD_TLS_HS_CONNECTED ) {
    ...
    /* OpenSSL client processes incoming messages and generates responses */
    if( hs->base.state != FD_TLS_HS_CONNECTED ) {
      _ossl_respond( ssl );
    }
  }
...
test_client( SSL_CTX * ctx ) {
  ...
  uchar tp_buf_local[] = { 0x01, 0x02, 0x47, 0xd0 };
  ulong tp_sz = 4UL;
  FD_TEST( 1==SSL_set_quic_tls_transport_params( ssl, tp_buf_local, tp_sz ) );
  ...
  *client = (fd_tls_t) {
  ...
  while( hs->base.state != FD_TLS_HS_CONNECTED ) {
    ...
    /* fd_tls client processes incoming messages and generates responses */
    if( hs->base.state != FD_TLS_HS_CONNECTED ) {
