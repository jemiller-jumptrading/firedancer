...
/* https://github.com/anza-xyz/agave/blob/v3.0.1/program-runtime/src/serialization.rs#L566-L653 */
static int
fd_bpf_loader_input_deserialize_aligned( fd_exec_instr_ctx_t * ctx,
  ...
  /* https://github.com/anza-xyz/agave/blob/v3.0.1/program-runtime/src/serialization.rs#L573 */
  ulong start = 0UL;

  uchar acc_idx_seen[256] = {0};

  start += sizeof(ulong); // number of accounts
  /* https://github.com/anza-xyz/agave/blob/v3.0.1/program-runtime/src/serialization.rs#L574-L650 */
  for( ushort i=0; i<ctx->instr->acct_cnt; i++ ) {
    uchar acc_idx = (uchar)ctx->instr->accounts[i].index_in_transaction;
    ...
    /* get the borrowed account
       https://github.com/anza-xyz/agave/blob/v3.0.1/program-runtime/src/serialization.rs#L584-L585 */
    fd_guarded_borrowed_account_t view_acc = {0};
    FD_TRY_BORROW_INSTR_ACCOUNT_DEFAULT_ERR_CHECK( ctx, i, &view_acc );

    if( FD_UNLIKELY( acc_idx_seen[acc_idx] ) ) {
      /* https://github.com/anza-xyz/agave/blob/v3.0.1/program-runtime/src/serialization.rs#L582 */
      start += 7UL;
    } else {
      /* https://github.com/anza-xyz/agave/blob/v3.0.1/program-runtime/src/serialization.rs#L586-L590 */
      acc_idx_seen[acc_idx] = 1;
      ...
      /* https://github.com/anza-xyz/agave/blob/v3.0.1/program-runtime/src/serialization.rs#L591-L593 */
      ...
      if( !stricter_abi_and_runtime_constraints ) {
        /* https://github.com/anza-xyz/agave/blob/v3.0.1/program-runtime/src/serialization.rs#L617-L627 */
        int err = 0;
        ...
        if( fd_borrowed_account_can_data_be_changed( &view_acc, &err ) ) {
          /* https://github.com/anza-xyz/agave/blob/v3.0.1/program-runtime/src/serialization.rs#L627-L633 */
          int err = fd_borrowed_account_set_data_from_slice( &view_acc, post_data, post_len );
          if( FD_UNLIKELY( err ) ) {
            return err;
          }
        ...
        /* https://github.com/anza-xyz/agave/blob/v3.0.1/program-runtime/src/serialization.rs#L633-L635 */
      ...
      /* https://github.com/anza-xyz/agave/blob/v3.0.1/program-runtime/src/serialization.rs#L636-L644 */
      if( !( stricter_abi_and_runtime_constraints && direct_mapping ) ) {
      ...
      /* https://github.com/anza-xyz/agave/blob/v3.0.1/program-runtime/src/serialization.rs#L645 */
...
/* https://github.com/anza-xyz/agave/blob/v3.0.1/program-runtime/src/serialization.rs#L404 */
static int
fd_bpf_loader_input_deserialize_unaligned( fd_exec_instr_ctx_t * ctx,
  ...
  for( ushort i=0; i<ctx->instr->acct_cnt; i++ ) {
    ...
    if( FD_UNLIKELY( acc_idx_seen[acc_idx] ) ) {
      ...
      /* https://github.com/anza-xyz/agave/blob/v3.0.1/program-runtime/src/serialization.rs#L436-L446 */
      if( !stricter_abi_and_runtime_constraints ) {
        int err = 0;
      ...
      /* https://github.com/anza-xyz/agave/blob/v3.0.1/program-runtime/src/serialization.rs#L455-L457 */
      if( !( stricter_abi_and_runtime_constraints && direct_mapping ) ) {
...
/* https://github.com/anza-xyz/agave/blob/v3.0.1/program-runtime/src/serialization.rs#L284-L311 */
int
