...
retry:
    ...
    if( fd_funk_txn_idx_is_null( parent_idx ) ) { i++; break; }
    next_xid = parent_xid;
  }
  ...
  if( cache->fork_depth < FD_PROGCACHE_DEPTH_MAX ) {
...
fd_progcache_search_chain( fd_progcache_t const *    cache,
  ...
  *out_rec = NULL;
  ...
  fd_funk_rec_map_shmem_private_chain_t * chain_tbl = (void *)( shmap+1 );

  /* Start a speculative transaction for the chain containing revisions
     of the program cache key we are looking for. */
  ulong ver_cnt = FD_VOLATILE_CONST( chain_tbl->ver_cnt );
  if( FD_UNLIKELY( ver_cnt & (1UL<<FD_FUNK_REC_MAP_CNT_WIDTH) ) ) {
  ...
  long            best_slot = -1L;
  for( fd_funk_rec_map_iter_t iter = fd_funk_rec_map_iter( cache->funk->rec_map, chain_idx );
  ...
  if( FD_UNLIKELY( FD_VOLATILE_CONST( chain_tbl->ver_cnt )!=ver_cnt ) ) {
...
fd_progcache_query( fd_progcache_t *          cache,
  ...
  if( rec ) {
    /* Record is too old (this branch is typically reached for published
       records) */
    if( FD_UNLIKELY( rec->pair.xid->ul[0] < epoch_slot0 ) ) rec = NULL;
  }
  return rec;
}
...
fd_progcache_peek( fd_progcache_t *          cache,
  ...
  if( entry->last_slot_verified < epoch_slot0 ) entry = NULL;
  return entry;
}
...
fd_progcache_lock_best_txn( fd_progcache_t * cache,
  ...
  if( target_slot <= last_publish->ul[0] ) return NULL; /* publishing record immediately */

  /* Scan fork graph newest to oldest */
  ulong target_xid_idx;
  ulong fork_depth = cache->fork_depth;
  for( target_xid_idx=0UL; target_xid_idx<fork_depth; target_xid_idx++ ) {
    if( target_xid_idx<=target_slot ) break;
  }

  /* Backtrack up to older forks */
  fd_funk_txn_t * txn;
  while( target_xid_idx>0 ) {
    /* Locate fork */
...
fd_progcache_insert( fd_progcache_t *                cache,
  ...
  if( FD_LIKELY( fd_sbpf_elf_peek( elf_info, progdata, progdata_sz, &config )==FD_SBPF_ELF_SUCCESS ) ) {
    ...
    rec = fd_progcache_rec_new( rec_mem, elf_info, load_slot, features, progdata, progdata_sz );
    if( !rec ) {
  ...
  if( !rec ) {  /* load fail */
    ...
    rec = fd_progcache_rec_new_nx( rec_mem, load_slot, load_slot );
  }
...
fd_progcache_pull( fd_progcache_t *           cache,
  ...
  ulong current_slot = env->slot;
  ulong epoch_slot0  = env->epoch_slot0;
  fd_progcache_rec_t const * found_rec = fd_progcache_peek( cache, xid, prog_addr, epoch_slot0 );
  ulong last_slot_modified = 0UL;
  if( found_rec ) {
    last_slot_modified  = found_rec->last_slot_modified;
    if( FD_LIKELY( last_slot_modified<found_rec->last_slot_verified ||
                   current_slot==found_rec->last_slot_verified ) ) {
      return found_rec;
    }
  }

  return fd_progcache_insert( cache, accdb, xid, prog_addr, env, last_slot_modified );
}
...
fd_progcache_invalidate( fd_progcache_t *          cache,
  ...
  fd_funk_rec_t const * prev_rec = fd_funk_rec_query_try_global( funk, xid, prog_addr, NULL, query );
