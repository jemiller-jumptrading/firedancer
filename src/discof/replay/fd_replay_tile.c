...
publish_vote_states( fd_replay_tile_t *  ctx,
                     fd_stem_context_t * stem,
                     fd_bank_t *         bank ) {
  fd_replay_votes_t * votes = fd_chunk_to_laddr( ctx->replay_votes->mem, ctx->replay_votes->chunk );
  fd_vote_states_t const * vote_states = fd_bank_vote_states_prev_locking_query( bank );
  fd_vote_states_iter_t iter_[1];
  ulong idx = 0UL;
  for( fd_vote_states_iter_t * iter = fd_vote_states_iter_init( iter_, vote_states );
  ...
  votes->vote_cnt = idx;
  ulong sz = sizeof(votes->vote_cnt)+(votes->vote_cnt*sizeof(votes->snapshot[0]));
  fd_stem_publish( stem, ctx->replay_votes->idx, 0UL, ctx->replay_votes->chunk, sz, 0UL, 0UL, fd_frag_meta_ts_comp( fd_tickcount() ) );
  ctx->replay_votes->chunk = fd_dcache_compact_next( ctx->replay_votes->chunk, sz, ctx->replay_votes->chunk0, ctx->replay_votes->wmark );
}
